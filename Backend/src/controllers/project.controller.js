...
export const getProjectByIdAndWorkspaceIdController = asyncHandler(
    async (req, res) => {
        const projectId = projectIdSchema.parse(req.parse.id);
        const workspaceId = workspaceIdSchema.parse(req.params.workspaceId)
        const userId = req.user?._id;
        const { role } = await getMemberRoleInWorkspace(userId, workspaceId)
        roleGuard(role, [Permissions.VIEW_ONLY])

        const { project } = getProjectByIdAndWorkspaceIdService(
            workspaceId,
            projectId
        );
...
export const deleteProjectController = asyncHandler (
    async(req, res) => {
        ...
        const workspaceId = workspaceIdSchema.parse(req.parse.workspaceId);
